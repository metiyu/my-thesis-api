...
def monte_carlo_simulation(returns_data, strategies, n_simulations=1000):
    results = []
    for sim_id in range(n_simulations):
        ...
        for strategy in strategies:
            ...
            try:
                weights = optimizer.optimize_portfolio(objective=strategy)
                ...
                results.append({
                    ...
                    "sharpe_ratio": metrics["modified_sharpe"],
                    "cvar": metrics["historical_cvar_95"],
                    "weights": [float(w) for w in weights.tolist()]
                })
            except Exception as e:
                print(f"Error pada simulasi {sim_id} strategi {strategy}: {e}")
    return results
...
if __name__ == "__main__":
    try:
        ...
        strategies = json.loads(sys.argv[5])  # ["modified_sharpe", "min_cvar"]
        ...
        simulation_results = monte_carlo_simulation(data, strategies, n_simulations)
